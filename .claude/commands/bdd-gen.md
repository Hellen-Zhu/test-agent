...
- **Optional path hint:** `--projectPath {path}` or `--e2eDir {path}` provides an automation project path hint for `bdd-agent` to resolve
...
- `bdd-agent` Phase 2 owns those decisions after Phase 1 test points are confirmed.
- `/bdd-gen` owns orchestration, review gates, writing approved files, and updating source state.

### If confirmed JSON contract mode:

Use the **Read** tool to read the JSON file generated by `/writeuserstories` or `/enrichstorydesign`. Do not validate the full story schema here; preserve the loaded content as the source payload for `bdd-agent`.
...
Perform only interface-level checks required to call `bdd-agent`:
...
Do not resolve `projectPath` / `{E2E_DIR}` in `/bdd-gen`. Collect path hints only and let `bdd-agent` resolve the automation project path.
...
Use the **Agent** tool to invoke `bdd-agent`. Pass the already loaded source payload unchanged with only the Phase 1 envelope:
...
pathHints:
  ...
  storyProjectHints: "{project/repo hints from source payload if present}"
instruction: "Read and follow the Phase 1 contract in ~/.claude/agents/bdd-agent.md."
```

Accept only the markdown report defined by `bdd-agent` Phase 1 Output.

Display the **complete** agent output to the user, followed by review guidance:

> {Insert the complete bdd-agent Phase 1 output here, unmodified.}
>
> ---
>
> **Review Checklist:**
...
Existing `.feature` style/file-mode/TC-sequence context is owned by `bdd-agent` Phase 2. Step definition/snippet/Java/helper implementation reuse is owned by `automation-agent`, not by `/bdd-gen` or `bdd-agent` Phase 2.

If Phase 2 reports `E2E_DIR` as `CONTEXT_GAP`, ask the user for an explicit path hint and re-run Phase 2 with that hint.

Use the **Agent** tool to invoke `bdd-agent`. Pass the same loaded source payload unchanged with the approved Phase 1 report and only the Phase 2 envelope:
...
pathHints:
  ...
  cwd: "{current working directory}"
  sourceReference: "{source JSON path or ADO reference}"
instruction: "Read and follow the Phase 2 contract in ~/.claude/agents/bdd-agent.md."
```

Accept only the markdown result defined by `bdd-agent` Phase 2 Output.

Display the **complete** agent output to the user, followed by review guidance:

> {Insert the complete bdd-agent Phase 2 output here, unmodified.}
>
> ---
>
> **Review Checklist:**
